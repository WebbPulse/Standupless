...
| `teams` | `/api/teams`, `/api/team-members`, `/api/statuses`, `/api/labels`, `/api/webhooks` | Per-team config: key prefix, estimate scale, transition rules, outbound webhook registration. |
...
| `planning` | `/api/cycles`, `/api/projects`, `/api/roadmap` | Time boxes, projects, cross-team roadmap read. |
...
| Read or write issues | all teams | all teams | all teams | invited teams only |
| Team config, transition rules, webhooks | yes | yes | team admin only | no |
...
- Guest reads go through the team membership check; a test asserts a guest 404s on a team it is not in.
...
| `memberships` | `workspace_id` | `member_key` (`user#<id>` or `team#<pid>#user#<uid>`) | `user_id-workspace_id-index` for "my workspaces" | none |
| `invites` | `workspace_id` | `invite_id` | `token_hash-index` | TTL `expires_at` |
| `teams` | `workspace_id` | `team_id` | `workspace_key_prefix-index` for prefix uniqueness | none |
| `team_config` | `workspace_id` | `config_key` (`team#<pid>#status#<sid>`, `#label#<lid>`, `#transition#<n>`) | none | none |
| `counters` | `workspace_id` | `counter_key` (`team#<pid>#issue`) | none | none |
| `issues` | `workspace_id` | `issue_id` | `ws_team-status_updated-index`, `ws_team-key_number-index`, `ws_assignee-updated_at-index`, `ws_team-cycle_id-index`, `ws_team-project_id-index`, `ws_parent-created_at-index` | stream `NEW_AND_OLD_IMAGES` |
...
**Issue keys.** `counters` holds one item per team. Allocation is a single `update` with `ADD next_number :one` and `return_values="UPDATED_NEW"`: atomic, no transaction. A crash between allocation and the issue write leaves a gap; gaps are accepted and nothing ever renumbers to close one. The human key `ABC-123` is denormalised onto the issue and indexed by `ws_team-key_number-index`.

**Access patterns per index.** `ws_team-status_updated-index` serves the board column and the list view by recency. `ws_assignee-updated_at-index` serves "my issues" across teams. `ws_parent-created_at-index` serves sub-issue listing and the rollup consumer. `ws_target-relation_type-index` serves "what blocks this" without a second write path. `ws_user-unread-index` is sparse, so the inbox badge counts a short index rather than filtering a scan.
...
4. **Key extraction.** Keys match case-insensitively against the team's own prefix from `teams`, never a global regex, over branch name, PR title, PR body and commit messages. A key naming a team the installation is not linked to is dropped.
...
6. **Transitions.** Per-team rules in `team_config`. Default: PR opened to In Progress, merged to Done, merged with a magic word closes. A transition never overrides a manual status change made after the PR event, which the consumer checks by comparing `updated_at`.
...
MVP tools: `search_issues`, `get_issue`, `create_issue`, `update_issue`, `add_comment`, `list_teams`, `list_statuses`, `assign_issue`. Deliberately no delete tool and no workspace administration.
...
| M1 | `workspaces`, `teams`, membership, roles, authz dependency, invariant tests | Tenancy fails closed before any content exists to leak |
...
| M4 | `planning`: cycles, projects, roadmap | Rollups without a second write path into issues |
