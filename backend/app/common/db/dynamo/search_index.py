...
partitioned per project so a search never reads across a project boundary and a
...
def search_partition(workspace_id: str, project_id: str) -> str:
    """The partition one project's postings live in, workspace first."""
...
def truncate_body(body: str | None) -> str:
    ...
    Indexing a whole body would let one long issue dominate a project's partition,
...
class SearchIndexRepository:
    """Maintains and reads the term postings of one project at a time."""

    def __init__(self, repository: Repository | None = None) -> None:
        """Take an injected package repository, or build this table's own."""
        self._repository = build_repository(SEARCH_INDEX, repository)

    def postings(self, workspace_id: str, project_id: str, term: str, *, limit: int = POSTING_CAP) -> list[str]:
        """Every issue id posted under one term in one project."""
        if not workspace_id or not project_id or not term:
            return []
        items = self._repository.iter_query(
            Key("ws_project").eq(search_partition(workspace_id, project_id)) & Key("term_doc").begins_with(f"{term}#"),
            max_items=limit,
        )
        return [str(item["term_doc"]).split("#", 1)[1] for item in items]

    def posting_count(self, workspace_id: str, project_id: str, term: str) -> int:
        """How many issues a term is posted against, counted no further than the cap."""
        return len(self.postings(workspace_id, project_id, term, limit=POSTING_CAP + 1))

    def add(self, workspace_id: str, project_id: str, term: str, issue_id: str) -> bool:
        """Post one issue under one term unless the term is already at its cap.

        A term past the cap is one that matches so much of a project that its
        ...
        bound.
        """
        partition = search_partition(workspace_id, project_id)
        if self.posting_count(workspace_id, project_id, term) >= POSTING_CAP:
            self.record_truncated(workspace_id, project_id, term)
            return False
        self._repository.put({"ws_project": partition, "term_doc": term_doc(term, issue_id)})
        return True

    def remove(self, workspace_id: str, project_id: str, term: str, issue_id: str) -> None:
        """Unpost one issue from one term."""
        self._repository.delete(
            {"ws_project": search_partition(workspace_id, project_id), "term_doc": term_doc(term, issue_id)}
        )
    ...
    def apply(
        ...
        *,
        ...
        for term in sorted(set(departed)):
            self.remove(workspace_id, project_id, term, issue_id)
        for term in sorted(set(appeared)):
            self.add(workspace_id, project_id, term, issue_id)

    def record_truncated(self, workspace_id: str, project_id: str, term: str) -> None:
        """Note that a term stopped taking postings in this project.

        Kept on one marker row per project so an operator can see which terms are
        answering partial results without a table scan.
        """
        self._repository.update(
            {"ws_project": search_partition(workspace_id, project_id), "term_doc": MARKER_SORT_KEY},
            update_expression="ADD #truncated :term",
            expression_names={"#truncated": "truncated"},
            expression_values={":term": {term}},
        )

    def truncated_terms(self, workspace_id: str, project_id: str) -> set[str]:
        """Every term that has stopped taking postings in this project."""
