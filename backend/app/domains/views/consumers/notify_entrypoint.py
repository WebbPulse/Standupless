"""The notify consumer's entrypoint, run as its own Lambda function.

Same image as the `views` routes, a different command. The function is behind a
...
DOMAIN = Domain(
    ...
    title="Standupless views notify consumer",
    load_routers=lambda: [],
...
def _routers() -> "list[APIRouter]":
    """The notify consumer's router alone."""
    from app.domains.views.consumers.notify import build_router
