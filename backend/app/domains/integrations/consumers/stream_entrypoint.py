"""The outbound consumer's entrypoint, run as its own Lambda function.
...
DOMAIN = Domain(
    ...
    title="Standupless integrations stream consumer",
    load_routers=lambda: [],
...
def _routers() -> "list[APIRouter]":
    """This consumer's router alone, handling the issues and comments streams."""
    from app.domains.integrations.consumers.stream import build_router
