"""The webhook-dispatch consumer's entrypoint, run as its own Lambda function.

Same image as the `integrations` routes, a different command. The function is
behind its event source mapping and no gateway route, so it serves the consumer
route and the root probes and nothing else: building the domain's routers here
would give a consumer an authorized read surface it has no reason to carry.
"""
...
SERVED = DOMAINS["integrations"]

DOMAIN = Domain(
    name="integrations-dispatch-consumer",
    title="Standupless integrations dispatch consumer",
    load_routers=lambda: [],
...
"""A descriptor carrying the `integrations` bundle and only this consumer's route.
...
def _routers() -> "list[APIRouter]":
    """This consumer's router alone, handling the write-back and outbound webhook jobs."""
    from app.domains.integrations.consumers.dispatch import build_router
