"""Attaching an issue to a cycle or a milestone, from the issues domain's side.

M4 adds `cycle_id` and `milestone_id` to the issue body. The properties worth
holding are that a target from another project is refused at the write rather than
...
def test_an_issue_carries_its_cycle_and_milestone(
    ...
    assert issue["milestone_id"] == milestone["milestone_id"]


def test_a_cycle_from_another_project_is_refused(client: TestClient, issues_client: TestClient, workspace: str) -> None:
    """A foreign cycle would count the issue into a bar its own team never sees."""
    sign_in(client, OWNER)
    foreign = seed_cycle(client, workspace, project_id=OTHER_PROJECT)

    sign_in(issues_client, OWNER)
    response = issues_client.post(
        f"/api/workspaces/{workspace}/issues",
        json={"project_id": PROJECT, "title": "Wrong cycle", "cycle_id": foreign["cycle_id"]},
    )

    assert response.status_code == 422


def test_a_milestone_that_does_not_exist_is_refused(issues_client: TestClient, workspace: str) -> None:
    """The target is read back rather than trusted."""
    sign_in(issues_client, MEMBER)

    response = issues_client.post(
        f"/api/workspaces/{workspace}/issues",
        json={"project_id": PROJECT, "title": "No such", "milestone_id": "01JB0000000000000000NOPE1"},
    )
...
def test_an_unattached_issue_writes_no_attribute(issues_client: TestClient, repositories: Any, workspace: str) -> None:
    ...
    assert "milestone_id" not in item
...
def test_the_list_filters_on_the_attachment(client: TestClient, issues_client: TestClient, workspace: str) -> None:
    ...
    response = issues_client.get(
        f"/api/workspaces/{workspace}/issues",
        params={"project_id": PROJECT, "cycle_id": cycle["cycle_id"]},
    )

    assert [row["title"] for row in response.json()["issues"]] == ["In the cycle"]


def test_a_patch_refuses_a_cycle_from_another_project(
    client: TestClient, issues_client: TestClient, workspace: str
) -> None:
