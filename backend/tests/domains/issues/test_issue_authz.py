...
a non-member 404s on the workspace, and a guest 404s on any team they hold no
...
def issue(client: TestClient, workspace: str, statuses: Any) -> "dict[str, Any]":
    """One issue in the team the guest can see, created by the owner."""
...
def hidden_issue(client: TestClient, workspace: str, statuses: Any) -> "dict[str, Any]":
    """One issue in the team the guest holds no membership in."""
    sign_in(client, OWNER)
    created = create_issue(client, workspace, team_id=OTHER_TEAM, title="Hidden")
    sign_out(client)
    return created
...
def test_every_workspace_role_reads_and_writes_issues(
    ...
    """An owner, an admin and a member all reach every team of the workspace."""
...
def test_a_guest_reaches_the_team_it_is_a_member_of(
    client: TestClient, workspace: str, statuses: Any, issue: Any
) -> None:
    """A team membership is what a guest's access is made of."""
    sign_in(client, GUEST)

    assert client.get(f"/api/workspaces/{workspace}/issues/{issue['id']}").status_code == 200
    assert create_issue(client, workspace, title="By the guest")["team_id"] == TEAM


def test_a_guest_sees_a_team_it_is_outside_as_absent(client: TestClient, workspace: str, hidden_issue: Any) -> None:
...
def test_a_guest_cannot_create_in_a_team_it_is_outside(client: TestClient, workspace: str, statuses: Any) -> None:
    """A create names its team in the body, so the same rule has to hold there."""
    sign_in(client, GUEST)

    response = client.post(
        f"/api/workspaces/{workspace}/issues",
        json={"team_id": OTHER_TEAM, "title": "Sneaking in"},
    )
    assert response.status_code == 404


def test_a_guest_list_omits_the_team_it_is_outside(
    client: TestClient, workspace: str, statuses: Any, issue: Any, hidden_issue: Any
) -> None:
    """The fan-out reads only the visible teams rather than filtering after."""
    sign_in(client, GUEST)
...
def test_a_guest_cannot_look_up_a_hidden_team_by_key(client: TestClient, workspace: str, hidden_issue: Any) -> None:
    """The key names the team, so the lookup has to apply the same rule."""
    sign_in(client, GUEST)

    assert client.get(f"/api/workspaces/{workspace}/issues/by-key/XYZ-1").status_code == 404


def test_a_team_admin_deletes_anyone_s_issue(client: TestClient, workspace: str, statuses: Any, issue: Any) -> None:
    """A workspace admin implies team admin, which the contract states."""
...
def test_a_guest_cannot_delete_an_issue_in_a_team_it_is_outside(
...
def test_an_assignee_must_be_able_to_see_the_team(client: TestClient, workspace: str, statuses: Any) -> None:
    """Assigning a guest an issue they cannot open would hide it from them."""
    sign_in(client, OWNER)

    response = client.post(
        f"/api/workspaces/{workspace}/issues",
        json={"team_id": OTHER_TEAM, "title": "Assigned", "assignee_id": GUEST},
    )
...
def test_an_assignee_outside_the_workspace_is_refused(client: TestClient, workspace: str, statuses: Any) -> None:
    ...
    response = client.post(
        f"/api/workspaces/{workspace}/issues",
        json={"team_id": TEAM, "title": "Assigned", "assignee_id": OUTSIDER},
    )
    assert response.status_code == 422
